...
class ReflectSamplingNeRFModel(Model):
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        density_outputs_prop, _ = self.prop.get_density(ray_samples_reciprocal)
        ...
        # renderer(weights, weights)
        accumulation = self.renderer_accumulation(weights)
        depth = self.renderer_depth(weights, ray_samples_pdf)
        mask_acc = (accumulation>1e-2).reshape(-1)
               
        # back ground
        background_color = self.background_color.to(weights_prop.device).expand(accumulation.shape[:-1]+self.background_color.shape[-1:])
        ...
        rgb = self.renderer_rgb(rgb_outputs, weights, background_color=background_color)

        diff = self.renderer_rgb(diff_outputs, weights, background_color=background_color)
        tint = self.renderer_rgb(tint_outputs, weights, background_color=background_color)
        ...
        outputs = {
            "rgb": rgb,
            "ref_rgb": background_color*(1.0-accumulation),
        ...
        ref_background_color = ref_background_color*accumulation[mask, :] + background_color[mask, :]*(1.0-accumulation[mask, :])
        ...
        # print debug       
        print(mask[mask].shape, mask[mask_acc].shape, mask[mask_culling].shape)
        ...
        print("accumulation :",torch.quantile(accumulation.cpu(), q=q).detach().numpy())
        print("depth        :",torch.quantile(depth.cpu(), q=q).detach().numpy())
