...
class ReflectSamplingNeRFNerfField(Field):
    ...
    def contract(
        ...
        mask = (norm>1).squeeze(-1)
        mean_contract = torch.where(mask[...,None], (2*norm-1)/norm2*mean, mean)
        ...
        jacobian = torch.where(mask[...,None,None], ((2*norm-2)*(eyes-outer)+eyes)/norm2, eyes)
        ''' J*cov*J.T :(J.T=J)'''
